    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeRequests() // 요청 인증하기
                .antMatchers("/", "/auth/**", "/js/**", "/image/**", "/dummy/**") // 이 경로로 들어오는 요청에 대해
                .permitAll() // 허용함
                .anyRequest() // 그 밖의 경로의 요청은
                .authenticated() // 인증되어야함
            .and() // 그리고
                .formLogin() // 폼로그인을 할거야
                .loginPage("/auth/login-form") // 폼로그인 경로는 여기야
                .loginProcessingUrl("/auth/login-proc") // 스프링 시큐리티가 해당 경로로 요청오는 로그인을 가로채서 대신 로그인 해준다
